...
def moea_influence_maximization(G, p, no_simulations, model, population_size=100, offspring_size=100,
								max_generations=100, min_seed_nodes=None, max_seed_nodes=None, n_threads=1,
								random_seed=None, initial_population=None, population_file=None):
...
def nsga2_generator(random, args):
	...
	logging.debug(
		"Creating individual of size %d, with genes ranging from %d to %d" % (individual_size, nodes[0], nodes[-1]))
...
if __name__ == "__main__":
	...
	G = nx.generators.barabasi_albert_graph(100, 3, seed=0)
...
seed_sets = moea_influence_maximization(G, p, no_simulations, model, population_size=16, offspring_size=16,
										random_seed=random_seed, max_generations=max_generations, n_threads=n_threads)
