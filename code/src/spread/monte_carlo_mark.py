...
def MonteCarlo_simulation(G, A, p, no_simulations, model, random_generator=None):
        ...
        if model == 'WC':
                for i in range(no_simulations):
                        results.append(WC_model(G, A, random_generator, mark=True))
        elif model == 'IC':
                for i in range(no_simulations):
                        results.append(IC_model(G, A, p, random_generator, mark=True))
...
if __name__ == "__main__":
        ...
        print("Simulation with IC model:")
        print(MonteCarlo_simulation(G, [0, 2, 4, 6, 8, 10], 0.7, 100, 'IC', random.Random(0)))
        print("Simulation with WC model:")
        print(MonteCarlo_simulation(G, [0, 2, 4, 6, 8, 10], 0.7, 100, 'WC', random.Random(0)))
